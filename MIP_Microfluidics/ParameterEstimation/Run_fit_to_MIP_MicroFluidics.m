function [] = Run_fit_to_MIP_MicroFluidics( resultBase, numExperiments, dattp)
...
Folder='.';
filePattern = fullfile(Folder, strcat('PlateReaderPE_Reduced_2','-','*.mat'));
...
parfor epcc_exps=1:numExperiments
    ...
    m = ['PlateReaderPE_Reduced_2-',num2str(epcc_exps),'.mat'];
    if ~ismember(m,fi)
    %
        try
            global_theta_guess = ParFull(epcc_exps,:);
            epccOutputResultFileNameBase = [resultBase,'-',num2str(epcc_exps)];
            [out] = fit_to_MIP_Microfluidics(epccOutputResultFileNameBase,epcc_exps,global_theta_guess,dattp);
        catch err
            %open file
