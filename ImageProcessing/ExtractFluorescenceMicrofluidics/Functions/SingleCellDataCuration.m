...
function [dat,sct6] = SingleCellDataCuration(direct, ident, pat3)
...
for i=1:length(sct2)
    ...
    tv = sctEDGE2{i};
    for j=1:maxid
        if j+1<maxid && tv(j)~=tv(j+1) && ~isnan(tv(j))
            iv(j:end)=NaN;
        end
    end
    sctNoEdge{i}= iv;
...
for i=1:length(sct2)
    iv = sct2{i}(1,:);
    for j=1:nf
        if j-1>1 && j+1<maxid && abs(iv(j)-iv(j+1))>10 && isnan(iv(j-1))
            iv(j)=NaN;
        elseif j-1>1 && j+3<maxid && abs(iv(j)-iv(j+3))>10 && isnan(iv(j-1))
...
for i=1:length(sct2)
    ...
    for j=20:nf
        if j-1>1 && j+1<maxid && abs(iv(j)-iv(j+1))>5 && isnan(iv(j-1))
            iv(j)=NaN;
        elseif j-1>20 && j+4<maxid && abs(iv(j)-iv(j+4))>5 && isnan(iv(j-1))
            iv(j)=NaN;iv(j+1)=NaN;iv(j+2)=NaN;
        end
    end
    sctNENB{i}= iv;
...
for i=1:max(n)
    A = sctNENBNBu{i}(1,:);
    if length(A(~isnan(A)))>round(maxid*0.2) && ~ismember(i,larf) && ~ismember(i,lar)
        f = f+1;
        sct6{f} = sctNENBNBu{i};
